...
#
# Runs Path A (bootstrap completes when goc + Bash(goc:*) are pre-set) and
# Path B (preflight routes to bootstrap when goc is denied) against the
...
run_path_a() {
    echo "=== Path A: bootstrap completes against fresh repo ==="
...
1. Invoke Skill(extend-deck). Its preflight should route you to Skill(bootstrap).
2. Run Skill(bootstrap) to completion. goc is on PATH and Bash(goc:*) is allowed.
3. Verify .game-of-cards/deck/ exists, then write 'A:passed' to result.txt.
" \
        --plugin-dir "$PLUGIN_DIR" \
        --permission-mode dontAsk \
        --allowedTools "Read,Write,Edit,Bash(cd:*),Bash(ls:*),Bash(pwd:*),Bash(goc:*),Bash(git:*),Bash(which:*),Skill(bootstrap),Skill(extend-deck)" \
        --max-turns 30 )
...
run_path_b() {
    echo "=== Path B: preflight routes to bootstrap when goc is not allowed ==="
...
1. Invoke Skill(extend-deck). !-blocks calling goc will be denied.
2. Per the preflight section, invoke Skill(bootstrap).
3. Bootstrap should surface verbatim remediation text telling the user
...
" \
        --plugin-dir "$PLUGIN_DIR" \
        --permission-mode dontAsk \
        --allowedTools "Read,Write,Bash(cd:*),Bash(ls:*),Bash(pwd:*),Bash(which:*),Skill(bootstrap),Skill(extend-deck)" \
        --max-turns 20 )
