...
const GocToolParams = Type.Object({
    verb: Type.Union(GOC_VERBS.map((v) => Type.Literal(v)), {
        description: "The goc subcommand to run. Most card lifecycle work uses `new`, `status`, `done`, `decide`, `advance`, `show`, or `triage`.",
    }),
    args: Type.Array(Type.String(), {
        description: "Positional and flag arguments forwarded to the verb (e.g., ['my-card-title', '--decision', 'X', '--because', 'Y']).",
        default: [],
    }),
    ...
    cwd: Type.Optional(Type.String({
        description: "Working directory for the goc invocation. Defaults to the active workspace root.",
    })),
});
function buildArgs(input) {
    ...
    const f = input.flags ?? {};
    if (f.tag)
        flagTokens.push("--tag", f.tag);
    if (f.status)
        flagTokens.push("--status", f.status);
    if (f.contribution)
        flagTokens.push("--contribution", f.contribution);
    if (f.worker)
        flagTokens.push("--worker", f.worker);
    if (f.board)
        flagTokens.push("--board");
    if (f.json)
        flagTokens.push("--json");
    if (f.since)
        flagTokens.push("--since", f.since);
...
async function resolveDeckDir(projectDir) {
...
const PATTERN_REMINDER = "[GoC | pattern-check] Before yielding: did your recent change touch a pattern with broader applicability? " +
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        async function runGoc(args, cwd) {
            ...
            const result = await api.runtime.system.runCommandWithTimeout("python3", ["-m", "goc.cli", ...args], {
                cwd,
                env,
                timeoutMs: 60_000,
            });
        ...
        api.registerTool({
            name: "goc",
            description: "Game of Cards deck CLI. Files, advances, decides on, or closes cards in `.game-of-cards/deck/`. " +
        ...
        api.on("before_prompt_build", async (ctx) => {
            ...
            if ((hasExploration || hasTooling) && !hasWork)
                return;
            if (!hasWork)
                return;
        ...
        api.on("agent_end", async (ctx) => {
            const projectDir = ctx?.projectDir ?? process.cwd();
            if (await isOptedOut(projectDir))
                return;
            if (ctx?.config?.pattern_generalization_check === false)
                return;
            ...
            const mutating = toolCalls.some((tc) => {
                if (CODE_MUTATING_TOOLS.has(tc?.name))
                    return true;
