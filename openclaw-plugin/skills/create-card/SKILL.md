...
`log.md`, and a placeholder DoD that the `finish-card` skill will refuse
...
Slug pattern: `^[a-z0-9][a-z0-9-]*[a-z0-9]$` (per the `card-schema` skill).
...
(see the `card-schema` skill "Title antipatterns"): the regex set rejects
...
- `--gate none` — fully mechanical: sed-style rename, stale-reference
  patch, single-line constant correction. the `next-card` skill can
...
- `--gate decision` — two or more credible fix paths and a human
  ...
  section in the body** (per the `card-schema` skill's Decision-gate
  body contract).
...
the `decide-card` skill's "When an agent invokes this skill" section
...
alone. See the `card-schema` skill "Draft" for the full contract.
...
journal). See the `card-schema` skill's "What goes where" subsection.
...
- **Refine the DoD** — replace the placeholder with real criteria.
  ...
  so the closure semantic is legible — see the `card-schema` skill
...
- the `card-schema` skill — field semantics, enum values, canonical
  tags, decision-gate body contract.
- the `scan-deck` skill — dedup queries for the open / done / disproved
  queues.
- the `advance-card` skill — if the body reveals the gate should be
