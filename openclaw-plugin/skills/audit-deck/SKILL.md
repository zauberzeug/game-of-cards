...
(rubric-derived)` body section (see the `create-card` skill Step 3).
...
3. **Hand to the `create-card` skill** for the actual filing
...
- the `create-card` skill — actual filing (title, scaffold, body, DoD).
- the `advance-card` skill — for `disproved` flips during
  verification.
- the `scan-deck` skill — dedup queries against existing queues
