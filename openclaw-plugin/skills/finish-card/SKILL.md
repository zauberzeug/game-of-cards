...
divert to the `advance-card` skill for `disproved` / re-`open` instead.
...
- If the fix attempt failed → revert, append a `## Disproved fix
  ...
  or flip back to `open` via the `advance-card` skill.
...
(preferred), or divert to the `advance-card` skill (with `<title> open`) and append
...
the `card-schema` skill's "What goes where" subsection.
...
override (per the `advance-card` skill Step 5).
...
- the `advance-card` skill — divert here if verification reveals the
