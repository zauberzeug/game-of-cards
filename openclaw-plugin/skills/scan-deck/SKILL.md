...
calls the `decide-card` skill per parked card to lower gates.
...
suggest the `scan-deck` skill (with `decisions to make`)**, which walks every
decision-gated card via AskUserQuestion in one round and records
each answer via the `decide-card` skill.

## Mode B — "decisions to make" (interactive Q&A)

Triggered by the `scan-deck` skill (with `decisions to make`) (or matching
...
### Step 4 — call the `decide-card` skill per selection
...
(Or invoke the `decide-card` skill if the human prefers the skill
indirection.)
...
- the `decide-card` skill — the human's one-action handoff for parked
...
- the `next-card` skill — auto-pick the highest-leverage card to *work*
  on (`gate=none` only). Use after triage when you want to take
  something off the queue yourself, not just look.
- the `audit-deck` skill — discovery hunt. Use when the queue feels
  thin or you suspect undocumented defects.
- the `create-card` skill — file a new card. Use when you spotted
  something during scan that isn't in the queue yet.
- the `card-schema` skill — schema reference. Use when filter results
