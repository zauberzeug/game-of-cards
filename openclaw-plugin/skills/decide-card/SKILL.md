...
- **Decision changes scope.** "Scope this differently / split /
  ...
  the `advance-card` skill (with `<title> superseded`) +
  the `create-card` skill for the replacement.
...
- Does NOT close the card (that's the `finish-card` skill's DoD-gated
  contract).
...
- the `scan-deck` skill — surfaces parked cards; its "decisions to make"
  Q&A calls this skill per card.
- the `advance-card` skill — status machine + `waiting_on` overlay; gate
  ≠ status.
