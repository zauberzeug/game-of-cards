...
the `create-card` skill / the `advance-card` skill / the `finish-card` skill.
...
survive every transition (artifacts: the `create-card` skill Step 7).
...
(the `finish-card` skill "After closure").
...
problem*; the `refine-deck` skill's quality pass checks the same
