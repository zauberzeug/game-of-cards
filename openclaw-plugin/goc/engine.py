...
def remove_frontmatter_field(text: str, field_name: str) -> str:
    """Line-anchored removal of `field: <whatever>` (and any block continuation).

    Inverse of `mutate_frontmatter_field`: drops the field's line(s) entirely
    rather than rewriting the value, so an optional flag returns to its absent
    (default) state without leaving `field: false` noise behind. No-op if the
    field is absent. Avoids a YAML round-trip (which would reorder keys), matching
    the sibling mutator's contract."""
    m = FRONTMATTER_RE.match(text)
    if not m:
        raise ValueError("no frontmatter found")
    fm_text = m.group(1)
    ...
    fm_text = pattern.sub("", fm_text, count=1)
    return f"---\n{fm_text}{trailing}---\n{body}"
...
def _load_consuming_repo_tags() -> set[str]:
    ...
    for match in _FENCED_YAML.finditer(extension_file.read_text()):
        # Guard the parse itself, not just the shape: `_FENCED_YAML`
        # matches every ```yaml block in the file, and the vendored
        # `yaml_lite` parser raises `ParseError` (a `ValueError`) on YAML
        # features it doesn't support (folded `>` scalars, anchors/aliases,
        # tab indentation). An unparseable illustrative block would
        # otherwise crash every goc command via `load_schema()` at import
        # time. Skip the bad block; sibling blocks still contribute. This
        # mirrors the `try/except` guards in `load_deck_config`,
        # `parse_frontmatter`, and `_resolve_deck_root`.
        try:
            block = yaml.safe_load(match.group(1)) or {}
        except Exception:
            continue
        # Guard the block shape too: a non-mapping block (a bare list, a
        # scalar) has no `.get` — calling it would crash the same way.
...
class Card:
    ...
    @property
    def draft(self) -> bool:
        """True iff the card carries the explicit `draft: true` flag — an
        unauthored scaffold not yet released to the deck. Stamped by `goc new`;
...
def validate_card(t: Card, schema: Schema, all_titles: set[str]) -> list[str]:
    ...
    if "draft" in fm and fm["draft"] is not None:
        if not isinstance(fm["draft"], bool):
            errors.append(
                f"{t.title}: draft: {fm['draft']!r} must be a boolean (true/false)"
            )
        elif fm["draft"] is True and status_value in TERMINAL_STATUSES:
            errors.append(
                f"{t.title}: draft: true is invalid on a terminal card "
                f"(status={status_value!r}); a closed card cannot be an unauthored scaffold"
            )
...
# Generated by `goc new`; see `_cmd_new`. A card still carrying BOTH of these is
# an unauthored scaffold — nothing has been written yet.
SCAFFOLD_DOD_PLACEHOLDER = "- [ ] (replace with real criteria)"
SCAFFOLD_BODY_PLACEHOLDER = "(write the design doc here)"
...
def card_is_draft(card: Card) -> bool:
    """True iff the card carries the explicit `draft: true` flag — an unauthored
    scaffold not yet released to the deck. `goc new` stamps it; it clears on
    `goc publish` / `goc status active` / `goc done`.
    ...
    committable — a lingering placeholder body must not silently re-hide it. The
...
def _build_parser() -> argparse.ArgumentParser:
  ...
  goc new child-card --advances parent-card --commit""",
    ...
    p_publish = subparsers.add_parser(
        "publish",
        help="Release a draft card to the deck: clear its draft flag (refuses on an unauthored scaffold).",
    )
    p_publish.add_argument("title")
    p_publish.add_argument("--commit", action="store_true",
                           help="Force auto-commit for this publish.")
    p_publish.add_argument("--no-commit", action="store_true",
                           help="Skip auto-commit for this publish.")
...
def _cmd_done(args):
    ...
    # Closing proves the card is authored; clear any lingering draft flag so a
    # terminal card never carries `draft: true` (which `validate` rejects).
...
def _git_auto_commit(card_dirs: list[Path], message: str, *, exclude_draft: bool = True) -> bool:  # noqa: PLR0911
    ...
    With `exclude_draft` (the default), unauthored draft scaffolds
    (`card_is_draft`) are dropped from the commit set: a consumer with
    `workflow.auto_commit: true` must not publish a card that is still a
    placeholder — exactly the dedup/supersede race the draft state guards
    against. Verbs that legitimately commit a freshly-authored card clear the
    draft flag first (`goc publish` / `goc status active` / `goc done`), so the
    card is no longer a draft by the time it reaches here. The lone caller that
    opts out is `goc new --commit`, an explicit user action that must commit the
    new card together with its edge writes to avoid a half-edge (one endpoint
    committed, the other left untracked). External auto-commit services bypass
    goc entirely and must filter drafts themselves — see
    ...
    if exclude_draft:
        ...
        for d in card_dirs:
            try:
                ...
                c = None  # unparseable: keep it — the mutation still needs committing
...
def _cmd_status(args):
    ...
    if new_status in TERMINAL_STATUSES and t.human_gate != "none":
        ...
        sys.exit(2)
    if new_status in {"superseded", "disproved"} and card_is_draft(t):
        print(
            f"ERROR: {title}: is an unauthored draft scaffold; refusing to mark it "
            f"{new_status!r}. A placeholder has no authored scope to judge as a duplicate "
            f"or to disprove — this is exactly the dedup/supersede automation race the "
...
def _cmd_publish(args):
    """Release a draft card to the deck by clearing its `draft` flag.

    The explicit counterpart to the implicit draft-clear that `goc status
    active` and `goc done` perform: it releases an *authored* card to the queue
    without claiming it for work. Refuses on an unauthored scaffold (DoD + body
    still the generated placeholders) — there is nothing to publish until the
    card is written. No-op (with a note) on a card that is already non-draft."""
    title = args.title
    commit = args.commit
    no_commit = args.no_commit
    _validate_commit_flags(commit, no_commit)
    ...
    if not card_is_draft(t):
        ...
        return
    if is_placeholder_scaffold(t):
        print(
            f"ERROR: {title}: is still an unauthored scaffold (placeholder DoD and body). "
            f"Write the card's real DoD and body before publishing it — `goc publish` "
            f"releases authored work, it does not author it.",
            file=sys.stderr,
        )
        sys.exit(2)
    text = (card_dir / "README.md").read_text()
    text = remove_frontmatter_field(text, "draft")
    (card_dir / "README.md").write_text(text)
    print(f"{title}: published (draft flag cleared); now visible in the queue")
    commit_policy = _commit_override(commit, no_commit)
    if auto_commit_enabled(commit_policy):
        if _git_auto_commit([card_dir], f"deck: publish {title}"):
            print("  committed")
...
def _cmd_new(args):
    ...
    fm = {
        ...
        **({"summary": summary} if summary is not None else {}),
        ...
        "tags": list(tags),
        "draft": True,
        "definition_of_done": SCAFFOLD_DOD_PLACEHOLDER,
    }
    if worker:
        fm["worker"] = worker
    body = f"\n# {title}\n\n{SCAFFOLD_BODY_PLACEHOLDER}\n"
    ...
    if _commit_override(commit, no_commit) is True:
        ...
        # draft filter applies to the *automatic* commit paths instead.
        if _git_auto_commit(commit_targets, f"deck: new {title}", exclude_draft=False):
            print("  committed")
...
def _cmd_wait(args):
    ...
    shape: setting an overlay releases an active claim to open and clears its
    worker. `--clear` drops only the overlay.
    """
    ...
    if args.clear:
        ...
        print(
            ...
            + ("; active claim released to open" if released_claim else "")
        )
