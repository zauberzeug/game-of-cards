...
!`"$([ -n "$CLAUDE_PLUGIN_ROOT" ] && echo "$CLAUDE_PLUGIN_ROOT/bootstrap" || echo .codex/skills)/_goc-bootstrap.sh" show <title> 2>&1 | head -3`
...
!`"$([ -n "$CLAUDE_PLUGIN_ROOT" ] && echo "$CLAUDE_PLUGIN_ROOT/bootstrap" || echo .codex/skills)/_goc-bootstrap.sh" --status all 2>&1 | grep -i <near-name-fragment>`
