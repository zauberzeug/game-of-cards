...
- `goc/install.py:856-906` — `_strip_claude_vendored_harness`, called from the vendored→plugin migration cleanup path (`install.py:2082-2094`)
...
- `goc/install.py:890-893` — hook-file enumeration inside `_strip_claude_vendored_harness` (same shape)
...
documented at install.py:859-862 ("removes only the skill directories
...
hook-loop omission (install.py:890-893) is verified by reading —
