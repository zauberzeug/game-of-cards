...
- `goc/install.py:811-861` — `_strip_claude_vendored_harness`, called from the vendored→plugin migration cleanup path (`install.py:1821-1833`)
- `goc/install.py:1217-1255` — `_sync_skill_tree` `replace_skills` path, called from the vendored in-place refresh during `goc upgrade --keep-local-skills`
- `goc/install.py:845-848` — hook-file enumeration inside `_strip_claude_vendored_harness` (same shape)
...
`_sync_skill_tree(replace_skills=True)` (install.py:1239-1243) repeats
...
documented at install.py:814-817 ("removes only the skill directories
...
hook-loop omission (install.py:845-848) is verified by reading —
