...
definition_of_done: |
  ...
  - [ ] All six `yaml.safe_load` sites in `goc/engine.py` switch to the vendored parser (see survey table in epic body).
  - [ ] `pyyaml` is removed from `project.dependencies` in `pyproject.toml`.
  - [ ] Round-trip parity: every existing card under `.game-of-cards/deck/` survives parse → emit → parse without a byte-level diff.
  - [ ] `goc/schema.yaml`, `.game-of-cards/config.yaml`, and the legacy deck config all load identically to their PyYAML output.
  - [ ] Unsupported YAML syntax (anchors, aliases, multi-doc, flow nesting beyond `[a, b]` / `{k: v}`, folded `>` scalars) errors with `<file>:<line>: <reason>` — never silently mis-parses.
  - [ ] CI smoke matrix on Python 3.10–3.13 passes with `pyyaml` uninstalled (`pip install --no-deps`).
  - [ ] Round-trip smoke test in CI walks the deck and asserts byte-equality.
