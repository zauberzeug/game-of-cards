...
Sub-card of `ship-game-of-cards-as-cross-agent-cli`. This is the per-repo install flow — the runtime equivalent of `git init` for a methodology framework. Without it, the PyPI package is just a CLI with no on-disk presence in target repos.
...
- AGENTS.md output → `write-agentsmd-alongside-claudemd` (separate card; AGENTS.md is a different file with a different convention).
- Per-agent shim templates beyond Claude → `multi-agent-shim-which-agents-at-v1`.
- Bootstrap UX for the missing-`goc` case (i.e., what happens *before* the user runs `goc install`) → `bootstrap-error-when-cli-not-on-path`.
- Migrating phasor-agents off vendored `deck.py` → `migrate-phasor-agents-off-vendored-deckpy`.
