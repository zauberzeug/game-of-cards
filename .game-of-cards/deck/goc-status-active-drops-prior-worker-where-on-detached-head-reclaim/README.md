...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_auto_populate_worker` docstring at `goc/engine.py:5790` updated to match the chosen behavior.
...
`goc/engine.py:5817-5847` — `_auto_populate_worker`.
...
The function's own docstring at `goc/engine.py:5791-5796` codifies
the intent:
...
Triggered from `_cmd_status` at `goc/engine.py:5851` on every
