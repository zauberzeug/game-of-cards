...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_auto_populate_worker` docstring at `goc/engine.py:5840` updated to match the chosen behavior.
...
`goc/engine.py:5867-5897` — `_auto_populate_worker`.
...
The function's own docstring at `goc/engine.py:5841-5846` codifies
the intent:
...
Triggered from `_cmd_status` at `goc/engine.py:5901` on every
