...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `goc/engine.py:6840` (`filed = t.created or now`) is replaced per the recorded decision; if the original filing date is still desired in the body, it appears as inline prose, not as the header timestamp.
...
Change `goc/engine.py:6840` from:
