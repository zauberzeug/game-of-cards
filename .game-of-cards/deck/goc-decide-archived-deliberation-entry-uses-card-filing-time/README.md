...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `goc/engine.py:6839` (`filed = t.created or now`) is replaced per the recorded decision; if the original filing date is still desired in the body, it appears as inline prose, not as the header timestamp.
...
`goc/engine.py:6830-6858` — the `_cmd_decide` function, specifically
...
- `_cmd_attest` (`engine.py:5756, 3898-3901`) — `today = _utc_now_iso()`
...
Cited code: goc/engine.py:6786 — `filed = t.created or now`
```
...
Change `goc/engine.py:6839` from:
