...
`waiting_impedes(card)` (`goc/engine.py:2514`) answers "does this card's
...
| `--waiting` filter | `engine.py:3977` | `status not in TERMINAL_STATUSES and waiting_impedes(t)` (just fixed) |
...
| gated-leverage line | `engine.py:3442-3447` | `status == "open" and ... and not waiting_impedes(t)` (open-only) |
