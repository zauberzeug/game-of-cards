...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `.github/workflows/refine-deck.yml` passes `--model opus` in `claude_args`
  - [ ] MECHANICAL: `grep -rn -- "--model" .github/workflows/` shows no remaining `claude-fable-5` override
  - [ ] PROCESS: closed predecessor card re-pin-autonomous-workflows-to-fable-5-after-re-enable amended with a forward pointer to this card
