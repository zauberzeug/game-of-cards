...
definition_of_done: |
  - [ ] TDD: a regression test constructs active cards with `waiting_on: false` (and `true` / `yes` / `no` / `42`) and no `waiting_until`, asserting `engine.waiting_impedes` is False while the unfixed `deck_session_start._is_impeded` is True — fails before fix, passes after.
  - [ ] TDD: fix in `goc/templates/hooks/deck_session_start.py` mirrors the engine's `isinstance(v, str)` resolution for `waiting_on` only — a token in `_TRUE_SET ∪ _FALSE_SET` or matching `_INT_RE` resolves to None (not impeded). The `waiting_until` reader is NOT narrowed the same way (the engine's `waiting_until` property has no `isinstance` guard).
  - [ ] TDD: same widening ported into `openclaw-plugin/index.ts` `isImpeded` so the TS hook agrees with the engine on coerced bool/int `waiting_on`.
...
`uv run python .game-of-cards/deck/session-start-hook-treats-coerced-bool-or-int-waiting-on-as-impediment/reproduce.py`:

```
waiting_on:  'false' | engine.waiting_impedes=False | hook._is_impeded= True | card.waiting_on=None
waiting_on:   'true' | engine.waiting_impedes=False | hook._is_impeded= True | card.waiting_on=None
waiting_on:    'yes' | engine.waiting_impedes=False | hook._is_impeded= True | card.waiting_on=None
waiting_on:     'no' | engine.waiting_impedes=False | hook._is_impeded= True | card.waiting_on=None
waiting_on:     '42' | engine.waiting_impedes=False | hook._is_impeded= True | card.waiting_on=None

DIVERGENCE: hook reports impeded but engine does not for coerced values ['false', 'true', 'yes', 'no', '42']. Defect reproduces.
```
...
Mirror the engine's `isinstance(v, str)` resolution in the hook, scoped to
`waiting_on` only. The cleanest shape: have `_card_waiting_on` (not the shared
`_scalar_or_none`) drop a tail that the parser would coerce away from `str` —
a token in `_TRUE_SET ∪ _FALSE_SET` or matching `_INT_RE`. Do **not** apply
the same narrowing to `_card_waiting_until`: the engine's `waiting_until`
property (`goc/engine.py:697-701`) has no `isinstance` guard, so coerced
`waiting_until` values follow a different contract and the existing
unparseable-backstop tests pin that behavior.

Port the same narrowing to `openclaw-plugin/index.ts`: before the `waitingOn
!== ""` test, resolve a coerced bool/int token to the empty string.

**Do not apply the fix yet** — this card is the briefing; a future puller
implements it against the DoD above.
