...
definition_of_done: |
  - [ ] TDD: `reproduce.py` exits zero — `filter_cards` returns `[]` for the `'foo'` query against a card with bare-string `advances: 'foo-card-extended'`, instead of falsely matching by substring.
