...
definition_of_done: |
  - [ ] MECHANICAL: `goc/templates/skills/card-schema/schema.yaml` rewritten so its `optional_fields` list matches `goc/schema.yaml` byte-for-byte (adds `supersedes`, `superseded_by`, `worker`).
