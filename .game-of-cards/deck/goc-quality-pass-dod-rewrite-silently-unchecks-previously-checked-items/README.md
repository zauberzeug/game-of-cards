...
summary: "`_apply_dod_rewrite` (engine.py:4678) hardcodes `- [ ]` when reconstructing a DoD item whose LLM-proposed `fix` text lacks an explicit checkbox prefix. The documented LLM contract (in `_QUALITY_PROMPT_TEMPLATE` at engine.py:4521) asks for `fix: \"rewrite...\"` — just the body text — so a fix accepted for a previously-checked `- [x]` item silently flips it to `- [ ]`. Attested completion is erased, and the card's close-readiness drops without any user-visible signal."
status: open
stage: null
contribution: medium
...
`_apply_verdict_interactive` (`engine.py:4712-4762`) → user (or
