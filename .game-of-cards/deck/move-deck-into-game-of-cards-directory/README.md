...
definition_of_done: |
  - [x] CLI discovers cards from `.game-of-cards/deck` as the canonical deck path
  - [x] Existing root `deck/` installs either keep working through compatibility lookup or migrate through an explicit command/path
  - [x] `.goc-version` and other deck metadata move under `.game-of-cards/deck`
  - [x] `.game-of-cards/config.yaml` remains the documented config path, or an intentional `config.yml` alias/rename is implemented consistently
  - [x] `goc install` scaffolds new repos with `.game-of-cards/deck`
  - [x] `goc validate`, `goc new`, `goc status`, `goc done`, `goc advance`, `goc move`, and board/queue views work against the new path
  - [x] Tests cover new-path decks, old root decks, and mixed/conflicting deck locations
  - [x] This repo is migrated or has an explicit compatibility note
