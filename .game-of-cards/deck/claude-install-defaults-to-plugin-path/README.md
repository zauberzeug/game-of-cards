...
definition_of_done: |
  ...
  - [ ] Investigate during implementation whether `goc install` can shell out to `claude -c "/plugin install game-of-cards@game-of-cards"` (or equivalent) to invoke the plugin install directly without needing the in-session agent to run the slash command. If supported and safe, this collapses the bootstrap to a single confirmation. If not supported, fall back to the agent-instruction model above. Either path is acceptable — the DoD ticks if one of them works
