...
- `goc/engine.py:5187` — `if rebase.returncode != 0:` (the only race detection)
- `goc/engine.py:5223` — `print("  pushed (after rebase)"); return True` (no post-rebase check that the local claim commit survived)
