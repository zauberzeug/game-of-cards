...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_cmd_wait` in `goc/engine.py` carries a `TERMINAL_STATUSES` guard mirroring `_cmd_decide` (engine.py:4908), with an error message naming the replacement path (file a new card; supersede if the closed card needs revisiting).
...
`_cmd_status` (engine.py:5904+) and `_cmd_done` (engine.py:4803+) carry
...
(engine.py:6512) and `_cmd_unadvance` (engine.py:6534) also lack
