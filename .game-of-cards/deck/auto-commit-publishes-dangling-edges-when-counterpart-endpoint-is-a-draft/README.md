...
- `goc/engine.py:4987-4997` — `_git_auto_commit`'s `exclude_draft` filter.
- `goc/engine.py:5234-5237` — `_cmd_status`: `commit_targets = [card_dir]`
  plus `DECK_DIR / successor` for `superseded --by`.
- `goc/engine.py:6521` / `:5725` — `_cmd_advance` / `_cmd_unadvance`:
...
`engine.py:6237-6242`, `exclude_draft=False`) exists precisely "to avoid a
...
sites at `engine.py:6008`, `:5710`, `:5725`.
...
- Preview: change the filter at `engine.py:4987-4997` to all-or-nothing
