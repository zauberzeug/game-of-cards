...
- `goc/engine.py:4937-4947` — `_git_auto_commit`'s `exclude_draft` filter.
- `goc/engine.py:5234-5237` — `_cmd_status`: `commit_targets = [card_dir]`
  plus `DECK_DIR / successor` for `superseded --by`.
- `goc/engine.py:6471` / `:5725` — `_cmd_advance` / `_cmd_unadvance`:
...
`engine.py:6187-6192`, `exclude_draft=False`) exists precisely "to avoid a
...
sites at `engine.py:5958`, `:5710`, `:5725`.
...
- Preview: change the filter at `engine.py:4937-4947` to all-or-nothing
