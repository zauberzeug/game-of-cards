...
- `goc/engine.py:4991-5001` — `_git_auto_commit`'s `exclude_draft` filter.
- `goc/engine.py:5234-5237` — `_cmd_status`: `commit_targets = [card_dir]`
  plus `DECK_DIR / successor` for `superseded --by`.
- `goc/engine.py:6525` / `:5725` — `_cmd_advance` / `_cmd_unadvance`:
...
`engine.py:6241-6246`, `exclude_draft=False`) exists precisely "to avoid a
...
sites at `engine.py:6012`, `:5710`, `:5725`.
...
- Preview: change the filter at `engine.py:4991-5001` to all-or-nothing
