...
closed_at: "2026-06-23T19:48:27Z"
human_gate: none
advances: []
advanced_by: []
tags: [bug, api-contract]
definition_of_done: |
  - [x] TDD: reproduce.py exits zero (`parse_closed_since("99999999999w")` raises `SystemExit(2)`, not `OverflowError`)
  - [x] TDD: a regression test asserts an oversized window exits 2 with a `goc: error: --closed-since:` message; valid windows (`24h`, `7d`, `2w`) and absolute dates still parse
  - [x] MECHANICAL: the fix lives in `parse_closed_since` (`goc/engine.py`) and reuses the existing exit-2 error style
  - [x] PROCESS: plugin mirrors re-synced; `uv run goc validate` passes and the full `unittest` suite stays green
