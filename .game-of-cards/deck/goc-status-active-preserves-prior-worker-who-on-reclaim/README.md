...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_auto_populate_worker` docstring at `goc/engine.py:5682` updated to match the chosen behavior.
...
`goc/engine.py:5695-5711` — `_auto_populate_worker`.
...
The docstring at `goc/engine.py:5682-5683` codifies the intent:
...
Triggered from `_cmd_status` at `goc/engine.py:5834`, which calls
