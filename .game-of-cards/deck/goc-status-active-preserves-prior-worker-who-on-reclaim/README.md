...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_auto_populate_worker` docstring at `goc/engine.py:5434` updated to match the chosen behavior.
...
`goc/engine.py:5447-5463` — `_auto_populate_worker`.
...
The docstring at `goc/engine.py:5434-5435` codifies the intent:
...
Triggered from `_cmd_status` at `goc/engine.py:5586`, which calls
