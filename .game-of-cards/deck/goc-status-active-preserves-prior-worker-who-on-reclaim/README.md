...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_auto_populate_worker` docstring at `goc/engine.py:5846` updated to match the chosen behavior.
...
`goc/engine.py:5859-5875` — `_auto_populate_worker`.
...
Triggered from `_cmd_status` at `goc/engine.py:5998`, which calls
