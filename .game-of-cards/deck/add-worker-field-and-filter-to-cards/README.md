...
definition_of_done: |
  - [x] Schema (`goc/schema.yaml`) gains an optional `worker` field accepted on cards in ANY status (open, active, blocked, done, disproved, superseded)
  - [x] Field shape: either a flat string (`worker: gpu-rig`) or a nested mapping (`worker: {who: gpu-rig, where: feature/foo}`). Both forms parse to the same in-memory model; the flat form is sugar for `{who: <value>}`
  - [x] Semantics documented: `worker.who` is a free-form identifier — a person ("rodja"), a machine ("gpu-rig-3"), or a capability tag ("gpu-required", "rendering-expert"). No registry enforcement; agents and humans agree on conventions out-of-band (registry resolution stays on the parent design card)
  - [x] At filing time: `goc new <title> --worker <id>` sets the field. Cards filed without `--worker` have no worker designation
  - [x] At claim time: `goc advance <title> --status active` (and `Skill(pull-card)`) auto-populates `worker.where` from `git rev-parse --abbrev-ref HEAD` and refines/sets `worker.who` from `git config user.name` IF the card has no pre-existing worker. If a worker is already set (designation), the claim respects it and only adds `where`
  - [x] At close time: `worker` is NOT cleared. The field persists as a record of who worked on the card. (Historical detail of multiple claims still lives in `log.md`.)
  - [x] CLI override flags on `goc advance`: `--worker-who <id>` and `--worker-where <branch-or-path>` for cases where auto-detection is wrong (agent identity, server-side run that wants to advertise a different branch)
  - [x] `goc validate` accepts the field on any status; rejects malformed shapes (e.g. mapping without `who`, non-string values)
  - [x] `goc -v` shows `worker.who` (and `@ where` when set) per card row
  - [x] `goc --board` includes worker info in card cells
  - [x] `goc --worker <X>` filter: substring-match (or exact-match — pick) against `worker.who`. Combines with existing filters via AND. Documented in CLI help
  - [x] `goc triage` and `Skill(next-card)` respect a `GOC_WORKER` env var (or config) so an autonomous loop on a specific runner only sees its eligible cards by default
  - [x] Existing active card `support-external-game-of-cards-state-location` and any others currently active are migrated by hand to set `worker` correctly; documented in their `log.md`
  - [x] Schema example, CLAUDE.md / AGENTS.md card-authoring rules updated to describe `worker` semantics and capability-tag conventions
