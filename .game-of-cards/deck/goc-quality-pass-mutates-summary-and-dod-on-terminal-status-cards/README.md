...
definition_of_done: |
  ...
  - [ ] PROCESS: sibling sweep — confirm `_cmd_move` (engine.py:6473) does or does not need its own terminal-status guard; file a follow-up card if the audit reveals `goc move` standalone is also unguarded (the quality-pass path invokes `move` via subprocess and would benefit from defense-in-depth).
...
- `goc/engine.py:4479-6959` — `_apply_summary_rewrite` (no terminal guard)
- `goc/engine.py:3059-3076` — `_apply_dod_rewrite` (no terminal guard)
- `goc/engine.py:4527-4577` — `_apply_verdict_interactive` (orchestrator; no terminal guard)
...
Compare the recently-added guard in `_cmd_decide` (engine.py:4773):
...
`_cmd_move` (engine.py:6473) also lacks a terminal-status guard, so the title-rename branch in `_apply_verdict_interactive` (engine.py:4539-4552) would rename closed cards too — but that's a separate sibling defect (see DoD process step). The quality-pass entry point is the right place to gate Layer-2 LLM mutations.
...
- [goc-decide-accepts-decisions-on-already-closed-cards](../goc-decide-accepts-decisions-on-already-closed-cards/) (closed) — guard added at engine.py:4773
