...
definition_of_done: |
  ...
  - [ ] MECHANICAL: the `active → open` release transition in `_cmd_status` (`goc/engine.py:5233`) applies rung 1 — set `waiting_on` (reason naming the auto-release) plus a near-future `waiting_until` — when the card is released without reaching a terminal status and carries no impediment overlay yet.
...
- `_cmd_status` — `goc/engine.py:5233` (the release transition to hook)
- `waiting_impedes` — `goc/engine.py:2481` (the overlay predicate reused)
- `validate_waiting_overlay` — `goc/engine.py:2029` (surfaces elapsed waits)
...
of a property `waiting_impedes` already documents (`goc/engine.py:2481`):
...
- **Counting `## ` headings in `log.md`** has partial precedent (the
  `log-md-closure-entry` derived check, `goc/engine.py:5010`), but that
