...
definition_of_done: |
  ...
  - [ ] MECHANICAL: the `active → open` release transition in `_cmd_status` (`goc/engine.py:5905`) applies rung 1 — set `waiting_on` (reason naming the auto-release) plus a near-future `waiting_until` — when the card is released without reaching a terminal status and carries no impediment overlay yet.
