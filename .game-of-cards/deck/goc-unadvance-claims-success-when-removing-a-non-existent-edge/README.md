...
- `goc/engine.py:6278-6291` — `_mutate_pair`
...
Contrast with `_cmd_advance` (engine.py:6513), which validates the proposed
...
1. **`exit 2 + ERROR on stderr`** — matches `_cmd_advance`'s self-edge and
   cycle handling (engine.py:6521, 4393). Strictest: a no-op verb call is a
...
3. **`exit 0 + "no edge to remove" success line`** — silent acceptance of
   ...
   return (engine.py:6272). Least intrusive but loses information.
...
Alternative: have `_mutate_pair` (engine.py:6278) return `True` iff at least
