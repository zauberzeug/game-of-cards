...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_cmd_unadvance` (engine.py:6531) checks edge presence before mutating, OR `_mutate_pair` returns a "no-op" sentinel that `_cmd_unadvance` honors before printing the success line and entering the auto-commit branch.
...
- `goc/engine.py:6274-6287` — `_mutate_pair`
...
Contrast with `_cmd_advance` (engine.py:6509), which validates the proposed
...
1. **`exit 2 + ERROR on stderr`** — matches `_cmd_advance`'s self-edge and
   cycle handling (engine.py:6517, 4393). Strictest: a no-op verb call is a
...
3. **`exit 0 + "no edge to remove" success line`** — silent acceptance of
   ...
   return (engine.py:6268). Least intrusive but loses information.
...
Concrete change in `_cmd_unadvance` (engine.py:6531):
...
Alternative: have `_mutate_pair` (engine.py:6274) return `True` iff at least
