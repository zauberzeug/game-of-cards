...
- `goc/engine.py:6330-6343` — `_mutate_pair` (calls it via `--apply`, `advance`, `new`, `superseded --by`).
- `goc/engine.py:2126-2144` — `find_half_edges` (the detector; tolerates the bare string).
...
half-edge for it (`engine.py:2139-2141`):
...
- `goc repair-edges` (dry run) → traceback ending `ValueError: advanced_by: not a list` at `engine.py:6356 → 4911`.
