...
`_utc_now_iso()` (`_cmd_done` at engine.py:4856; `_cmd_status` for
`disproved`/`superseded` at engine.py:4954; `_auto_populate_worker`-adjacent
flips at engine.py:5997). So the offending input is **not** produced by
