...
- `goc/engine.py:1902-1904` — `validate_card`'s `closed_at` check:
...
- `goc/engine.py:1156` — `_is_iso_date` (the predicate behind every date
  field) is shape + calendar-validity only; no temporal-ordering.

## What's missing

`closed_at` is documented as "a single date per terminal exit" (see
`Card.closed_at` at `goc/engine.py:926-931`). A timestamp in the future
...
- `--done --since YYYY-MM-DD` (engine.py:3043) silently includes a card
  whose `closed_at` is `2099-12-31` for any `--since` in this century.
- `--closed-since WINDOW` (engine.py:3123) compares against `now -
...
`_utc_now_iso()` (`_cmd_done` at engine.py:4905; `_cmd_status` for
`disproved`/`superseded` at engine.py:5003; `_auto_populate_worker`-adjacent
flips at engine.py:6049). So the offending input is **not** produced by
...
Add a helper next to `_closed_at_instant` (engine.py:3165):
