...
`_utc_now_iso()` (`_cmd_done` at engine.py:4855; `_cmd_status` for
`disproved`/`superseded` at engine.py:4953; `_auto_populate_worker`-adjacent
flips at engine.py:5996). So the offending input is **not** produced by
