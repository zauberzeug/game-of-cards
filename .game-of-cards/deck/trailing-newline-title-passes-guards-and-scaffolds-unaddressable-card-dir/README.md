...
- `goc/engine.py:6187` — `_cmd_new`: `if not re.match(schema.title_pattern, title):`
- `goc/engine.py:6654` — `_cmd_move`: same check for the rename target
- `goc/engine.py:1849` — `validate_card`: same check on the stored title
...
- `goc status $'newline-tail\n' active` claims and auto-commits the
  malformed path.
- The `TITLE_ANTIPATTERNS` jargon guard (engine.py:6063) never fires:
