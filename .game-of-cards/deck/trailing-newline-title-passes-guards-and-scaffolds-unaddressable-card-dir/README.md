...
- `goc/engine.py:6191` — `_cmd_new`: `if not re.match(schema.title_pattern, title):`
- `goc/engine.py:6658` — `_cmd_move`: same check for the rename target
...
- `goc status $'newline-tail\n' active` claims and auto-commits the
  malformed path.
- The `TITLE_ANTIPATTERNS` jargon guard (engine.py:6067) never fires:
