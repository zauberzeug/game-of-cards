...
- `goc/engine.py:6113-6126` (`_mutate_pair` — the disk-write helper repair-edges shares with `advance` / `unadvance`).
...
(`engine.py:4303`). Anyone following that hint, or any `/loop` agent
...
`_cmd_move` at `engine.py:6473-6533` exercises a similar disk-write
...
subparser at `engine.py:3969-3975` also does not expose `--commit` /
