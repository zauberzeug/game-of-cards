...
- `goc/engine.py:6224-6237` (`_mutate_pair` — the disk-write helper repair-edges shares with `advance` / `unadvance`).
...
(`engine.py:4385`). Anyone following that hint, or any `/loop` agent
...
`_cmd_move` at `engine.py:6584-6644` exercises a similar disk-write
...
subparser at `engine.py:4036-4042` also does not expose `--commit` /
