...
definition_of_done: |
  ...
  - [ ] MECHANICAL: chosen guard / coercion lands at engine.py:6268 (or upstream in parse_frontmatter, per the decision above)
...
The only caller is `_mutate_pair(..., add=False)` at `engine.py:6286`, which
runs `op` on both the child's and the parent's README. `_mutate_pair(add=False)`
is reached exclusively from `_cmd_unadvance` (engine.py:6541) — the
`goc unadvance <title> --by <advancer>` verb. The fields exercised are
`advanced_by` on the child and `advances` on the parent (see engine.py:6541).
