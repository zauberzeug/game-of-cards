...
definition_of_done: |
  - [x] TDD: reproduce.py exits zero — a `- [ ]` line inside a fenced code block in `definition_of_done` is not counted as a DoD box (`count_dod_boxes` returns `(0, 1)` for the reproducer's DoD)
  - [x] TDD: a regression test in `tests/` exercises `count_dod_boxes`, `_dod_box_indices`, and `untagged_dod_items` against a DoD containing a fenced example checkbox and asserts all three agree on the non-fenced line set
  - [x] MECHANICAL: the three DoD scanners share one fence-tracking helper so they cannot drift apart on fence handling
...
Added a shared fence-tracking helper in `engine.py` and routed all
three scanners through it so they agree on which lines are "real"
checkbox lines (the existing comments already require the three to
agree):
...
`count_dod_boxes`, `_dod_box_indices`, and `untagged_dod_items` now
skip masked lines. Single-site (all within `engine.py`); the plugin
mirrors (`claude-plugin/`, `codex-plugin/`, `openclaw-plugin/`) were
regenerated by `scripts/sync_plugin_assets.py`. Regression coverage
lives in `tests/test_dod_fenced_checkbox.py`.
