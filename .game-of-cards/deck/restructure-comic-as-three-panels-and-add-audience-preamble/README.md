...
definition_of_done: |
  ...
  - [ ] README opens with an audience preamble that names who this is for and who it is NOT for yet, grounded in the persona list from `define-personas-and-use-cases-for-game-of-cards`
  - [ ] Preamble closes with the "it's just a to-do manager — the rest is automation around it" framing so readers cannot mis-position the tool
  - [ ] Ordering: preamble → comic → "how it works" → install. Install does not appear until the reader has the mental model
  - [ ] Audience copy aligns with persona names from the personas card (soft-block on that card if names need to match)
