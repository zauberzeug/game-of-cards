...
- `goc/install.py:1572` and `goc/install.py:1815` — `goc install` calls
  `_write_skills_source(target, ...)` immediately after copying the
  template config.
- `goc/install.py:1463-1493` — `_write_skills_source` rewrites the
  `skills_source:` line.
...
`skills_source: <value>` into config.yaml (`install.py:1572`/`:1780`),
