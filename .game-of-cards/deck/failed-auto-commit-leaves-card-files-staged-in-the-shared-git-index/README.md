...
definition_of_done: |
  ...
  - [ ] MECHANICAL: whichever of the exit-code / success-line options the decision picks is implemented consistently across all seven call sites (`engine.py:6017, 5763, 5959, 6216, 6238, 6253, 6582`), rather than in `_cmd_publish` alone.
...
- Seven auto-committing call sites reach it: `goc status` (`engine.py:6017`),
  `goc publish` (`:5763`), `goc new --commit` (`:5959`), `goc wait` (`:6216`),
  `goc advance` (`:6238`), `goc unadvance` (`:6253`), `goc decide` (`:6582`).
  `auto_commit_enabled` (`engine.py:5180-5188`) defaults to **true** whenever
...
half-finished git state on failure (`git rebase --abort`, `engine.py:5323`),
and `goc move`'s `git mv` falls back rather than stranding a side effect
(`engine.py:6683-6685`). `_git_auto_commit` is the outlier against a local
