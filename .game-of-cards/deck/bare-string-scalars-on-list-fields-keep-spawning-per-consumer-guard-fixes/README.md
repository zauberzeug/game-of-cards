...
`goc/engine.py:6104` — `_remove_from_list_field`:
...
Compare to the sibling `_add_to_list_field` at `engine.py:6091`,
...
`goc/engine.py:3803-3807` — the verbose (`-vv`) branch of
...
`_remove_from_list_field` is called from `_mutate_pair` (engine.py:6113),
which is called by:
