...
*does* reject this shape (`engine.py:1813`), but the read-time
...
`goc/engine.py:5856` — `_remove_from_list_field`:
...
Compare to the sibling `_add_to_list_field` at `engine.py:5843`,
...
`goc/engine.py:3635-3639` — the verbose (`-vv`) branch of
...
`_remove_from_list_field` is called from `_mutate_pair` (engine.py:5865),
which is called by:
