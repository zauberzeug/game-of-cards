...
*does* reject this shape (`engine.py:2023`), but the read-time
...
`goc/engine.py:6321` — `_remove_from_list_field`:
...
Compare to the sibling `_add_to_list_field` at `engine.py:6308`,
...
`goc/engine.py:3444-3448` — the verbose (`-vv`) branch of
...
`_remove_from_list_field` is called from `_mutate_pair` (engine.py:6330),
which is called by:
