...
definition_of_done: |
  ...
  - [ ] MECHANICAL: docstring at `goc/engine.py:5249` and kickoff doc at `goc/templates/skills/kickoff/reference.md:72-74` updated to match the chosen scope
...
- `goc/engine.py:4901` — `_cmd_done` calls `_enforce_closure_on_integration_or_exit(title)`.
- `goc/engine.py:4988` — `_cmd_done_bundle` calls the same helper inside its bundle loop.
