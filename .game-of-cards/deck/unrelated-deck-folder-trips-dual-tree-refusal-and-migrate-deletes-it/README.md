...
- `goc/engine.py:4153` — every verb except `migrate` exits 1 on the conflict, suggesting `goc migrate`
- `goc/engine.py:7015` — `legacy_dirs = {d.name: d for d in legacy.iterdir() if d.is_dir()}` (only dirs are ever copied)
- `goc/engine.py:7097` — `shutil.rmtree(legacy)` (deletes the whole folder, including never-copied top-level files)
