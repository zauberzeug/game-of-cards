...
definition_of_done: |
  ...
  - [ ] PROCESS: decision recorded on whether `_cmd_unadvance` rejects self-target like `_cmd_advance` (caller-level guard, mirrors engine.py:6516-6518) OR `_mutate_pair` learns to handle child==parent correctly (helper-level guard, also covers any future `_cmd_repair_edges` self-edge repair). The chosen path is reflected in the body's "Fix" section before implementation.
  - [ ] MECHANICAL: `_cmd_unadvance` (engine.py:6535) and/or `_mutate_pair` (engine.py:6278) carries the guard described in the decision. The output of `goc unadvance foo --by foo` matches the chosen failure mode (ERROR + exit 2, OR clean removal of both halves).
...
- `goc/engine.py:6278-6291` — `_mutate_pair` (reads `parent_text` before the first write, so child==parent makes the second write revert the first).
