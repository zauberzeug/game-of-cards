...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_cmd_unadvance` (engine.py:6534) and/or `_mutate_pair` (engine.py:6277) carries the guard described in the decision. The output of `goc unadvance foo --by foo` matches the chosen failure mode (ERROR + exit 2, OR clean removal of both halves).
