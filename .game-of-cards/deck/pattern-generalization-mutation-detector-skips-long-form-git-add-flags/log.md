...
## 2026-05-30: closure — reproduce.py before/after

**Before** (`_BASH_COMMIT_RE` with `-[A-Za-z]` alternation):
...
FAIL: 3 row(s) diverged from intended behavior.
```

**After** (`_is_broad_git_mutation` tokenizes with `shlex.split` and matches tokens against `{-A,-p,-u,--all,--update,--patch,.}`, rejects `--` separator and bare paths):
...
'git add -u'                 True       True       ok
'git add .'                  True       True       ok
'git add --all foo/'         True       True       ok
'git add --update'           True       True       ok
'git add --patch'            True       True       ok
'git add -- foo.py'          False      False      ok
'git add foo.py'             False      False      ok
'git commit -m msg'          True       True       ok

PASS: matcher covers long-form staging flags.
```

The TS port in `openclaw-plugin/index.ts` mirrors the same tokenize-and-match shape via a minimal `shellSplit` + `isBroadGitMutation` pair, replacing the previous regex pair. Auto-synced mirrors under `claude-plugin/hooks/`, `codex-plugin/hooks/`, and the dogfood copy at `.claude/hooks/` are regenerated by `scripts/sync_plugin_assets.py`.
