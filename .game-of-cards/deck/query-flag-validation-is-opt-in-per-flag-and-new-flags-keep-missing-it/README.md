...
- `goc/engine.py:4313` — `_cmd_default` presentation dispatch
  (`if args.board: ... elif args.as_json: ...`)
- `goc/engine.py:4230` — status auto-extend for `--waiting` /
...
The presentation dispatch (`goc/engine.py:4313`) was
...
status auto-extend (`goc/engine.py:4230`) fires only when
