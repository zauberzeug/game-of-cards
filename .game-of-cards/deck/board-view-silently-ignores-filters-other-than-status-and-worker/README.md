...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `engine.py:4314` updated to reflect the chosen rule. Every filter the table renderer honors must reach `render_board` (or be explicitly documented as out-of-scope in `--board`'s help text).
...
- `goc/engine.py:4313-4314` — the `--board` branch of `_cmd_default`:
...
- `goc/engine.py:4248` — `status_filter_explicit = bool(args.done_flag or args.status_flag is not None)`.
