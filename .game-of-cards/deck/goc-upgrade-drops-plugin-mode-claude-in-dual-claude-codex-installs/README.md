...
- `goc/install.py:1978-1979` — `upgrade()` consumes it: `default_agents = installed or DEFAULT_AGENTS`
- `goc/install.py:1856-1857` — `goc install` pins `skills_source` (`vendored` iff Claude vendored, else `plugin`)
...
`upgrade()` at `install.py:1978` reads `_detect_installed_surfaces` and drops
Claude.
...
1. **Read `skills_source` from `.game-of-cards/config.yaml`.** Clean and
   ...
   install, `install.py:1856`), so `skills_source: plugin` alone does NOT
