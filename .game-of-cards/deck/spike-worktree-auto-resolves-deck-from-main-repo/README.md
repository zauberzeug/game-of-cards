...
tags: [story, infra]
definition_of_done: |
  - [ ] Detect that the current working directory is a git worktree (not the primary working tree) — `git rev-parse --git-common-dir` differs from `git rev-parse --git-dir` is the standard signal
  - [ ] When in a worktree, `engine.DECK_DIR` resolves to `.game-of-cards/deck/` in the *common* working tree (one level above the worktree's git dir), not the worktree's own checkout
  - [ ] Behavior is opt-in via `.game-of-cards/config.yaml` (or env var) — some users may want a per-worktree deck (parallel experiments). Default for the spike is opt-in; promote to default if usage warrants it
  - [ ] Verified manually: create a worktree on a side-branch, run `goc` from it, see the same open queue as on main; claim a card from the worktree, see the claim from the main checkout
  - [ ] Documented in CLAUDE.md / AGENTS.md GoC blocks so agents working in worktrees know where the deck actually lives
  - [ ] Edge case: multiple worktrees racing on the same card. At minimum log a warning when claim fails because card is already active; full conflict resolution belongs in `design-claim-protocol-with-branch-and-author-metadata`
