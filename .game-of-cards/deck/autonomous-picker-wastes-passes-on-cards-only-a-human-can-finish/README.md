...
(`goc/engine.py:2660`). A heuristic there would inspect the placeholder
and find nothing. Any authoring-time detection must hook `_cmd_publish`
(`goc/engine.py:6096`) — where the authored DoD first reaches the queue,
...
state (the `log-md-closure-entry` derived check, `goc/engine.py:5702`),
