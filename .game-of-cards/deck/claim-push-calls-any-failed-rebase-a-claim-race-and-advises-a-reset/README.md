...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_git_claim_push_with_retry` (`goc/engine.py:5322`) no longer asserts a claim race from a bare `rebase.returncode != 0`, and never names a worker read off a card whose remote status is not `active`.
  - [ ] MECHANICAL: the remedy sentence at `goc/engine.py:5344` stops prescribing `reset to origin/<branch>` unconditionally — AGENTS.md "Parallel-Agent Commit Safety" forbids that cleanup class on a shared branch.
  - [ ] MECHANICAL: the `_git_claim_push_with_retry` docstring (`goc/engine.py:5266`) drops the premise "the rebase fails — meaning another worker modified the same card concurrently", which is the false inference this card is about.
...
- `_git_claim_push_with_retry` — `goc/engine.py:5266` (the function)
- the collapsing branch — `goc/engine.py:5322` (`if rebase.returncode != 0:`)
- the message and its remedy — `goc/engine.py:5344`
- the rival-identity read — `goc/engine.py:5336` (`worker = fm.get("worker")`)
- the caller that turns the verdict into `sys.exit(2)` — `goc/engine.py:6020`
- the opt-in — `claim_push_enabled`, `goc/engine.py:5251`
...
```python
if check.returncode == 1:          # goc/engine.py:5233 — genuinely not integrated
    ...
    sys.exit(2)
...
`False` return becomes `sys.exit(2)` (`goc/engine.py:6020`). `pull-card` runs
...
which is what the detached-HEAD branch (`goc/engine.py:5302`) already does,
...
- **Not a meta-fix.** The "collapse a subprocess exit code into one semantic
  ...
  fail toward surfacing the real error (`goc/engine.py:5062`, `:5052`). Two
