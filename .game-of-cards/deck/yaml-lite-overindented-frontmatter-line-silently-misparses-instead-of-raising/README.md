...
definition_of_done: |
  - [ ] TDD: reproduce.py exits zero — both over-indent cases raise ParseError instead of silently mis-parsing
  - [ ] TDD: a unit test in tests/ asserts ParseError is raised for an over-indented mapping key and for an over-indented bare-scalar continuation
  - [ ] TDD: existing valid block mappings, nested mappings, block sequences, and block scalars still parse unchanged (regression suite stays green)
...
In `_parse_block_mapping`, after the `curr < indent` break, reject a
more-indented line rather than treating it as a sibling:
