...
definition_of_done: |
  ...
  - [ ] MECHANICAL: `_cmd_attest` in `goc/engine.py` carries a `TERMINAL_STATUSES` guard mirroring `_cmd_decide` (engine.py:4855) and `_cmd_done` (engine.py:3243), with an error message naming the replacement path (re-open is forbidden; file a new card).
  - [ ] MECHANICAL: the misleading `Next: goc done <title> to close once all DoD items are ticked.` print at `engine.py:5787` is dropped on terminal-card paths (or removed entirely if the guard short-circuits before it runs).
...
`goc/engine.py:5683-5787` — `_cmd_attest`.
...
- `_cmd_decide` — `engine.py:4855` — `if t.status in TERMINAL_STATUSES: ...`
