...
- `goc/engine.py:631-646` — `replace_or_append_decision`. The two-branch logic (`if DECISION_REQUIRED_RE.search(body): sub` else `append`) never inspects the body for a pre-existing `## Decision` heading.
