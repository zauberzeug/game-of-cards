...
- `_coerce_config_bool` — `goc/engine.py:5086-5097` (the fallback)
- `auto_commit_enabled` — `goc/engine.py:5189` (`default=True`)
- `_enforce_closure_on_integration_or_exit` — `goc/engine.py:5210` (`default=False`)
- `claim_push_enabled` — `goc/engine.py:5264` (`default=False`)
- the "auto_commit is disabled" warning that never fires — `goc/engine.py:5193`
- `get_skills_source` — `goc/engine.py:5396-5408`, the sibling config reader
...
- **`auto_commit` (default True).** The user is trying to *stop* goc from
  ...
  configuration (`goc/engine.py:5193`) — "auto_commit is disabled but the
...
(`goc/engine.py:5396-5408`) does the same job for the `skills_source` key,
...
settles whether the three call sites (`goc/engine.py:5189`, `:5156`,
...
to a one-line change at `goc/engine.py:5097` with no call-site churn, and
this card is mechanical from there.

## Fix sketch (pending the decision above)

Replace the `return bool(value)` fallback at `goc/engine.py:5097` with
...
Replace the `return bool(value)` fallback at `goc/engine.py:5097` with the
