...
- `_coerce_config_bool` — `goc/engine.py:5085-5096` (the fallback)
- `auto_commit_enabled` — `goc/engine.py:5188` (`default=True`)
- `_enforce_closure_on_integration_or_exit` — `goc/engine.py:5209` (`default=False`)
- `claim_push_enabled` — `goc/engine.py:5263` (`default=False`)
- the "auto_commit is disabled" warning that never fires — `goc/engine.py:5192`
- `get_skills_source` — `goc/engine.py:5395-5407`, the sibling config reader
...
- **`auto_commit` (default True).** The user is trying to *stop* goc from
  ...
  configuration (`goc/engine.py:5192`) — "auto_commit is disabled but the
...
(`goc/engine.py:5395-5407`) does the same job for the `skills_source` key,
...
settles whether the three call sites (`goc/engine.py:5188`, `:5156`,
...
to a one-line change at `goc/engine.py:5096` with no call-site churn, and
this card is mechanical from there.

## Fix sketch (pending the decision above)

Replace the `return bool(value)` fallback at `goc/engine.py:5096` with
...
Replace the `return bool(value)` fallback at `goc/engine.py:5096` with the
