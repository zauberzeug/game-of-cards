---
title: goc-ship-game-of-cards-as-cross-agent-cli
...
human_gate: none
advances: []
advanced_by: [goc-package-pyproject-and-pypi-release, goc-install-command-scaffolds-repo, goc-write-agentsmd-alongside-claudemd, goc-multi-agent-shim-which-agents-at-v1, goc-bootstrap-error-when-cli-not-on-path, goc-migrate-phasor-agents-off-vendored-deckpy]
tags: [epic, infra, meta-fix]
definition_of_done: |
  - [ ] `game-of-cards` package published on PyPI; `pipx install game-of-cards` puts `goc` on PATH (sub-card: goc-package-pyproject-and-pypi-release)
  - [ ] `goc install` populates `.claude/skills/`, `deck/`, hooks, CLAUDE.md sections in any target repo (sub-card: goc-install-command-scaffolds-repo)
  - [ ] `goc install` writes/merges `AGENTS.md` so Codex/Cursor/OpenCode/Copilot/Aider see the methodology (sub-card: goc-write-agentsmd-alongside-claudemd)
  - [ ] `goc install --agents <list>` populates per-agent shims; v1 agent set decided and documented (sub-card: goc-multi-agent-shim-which-agents-at-v1)
  - [ ] When `goc` is missing from PATH, skills/hooks emit one-line `pipx install game-of-cards` instructions instead of cryptic shell errors (sub-card: goc-bootstrap-error-when-cli-not-on-path)
  - [ ] phasor-agents repo itself runs on PATH-resolved `goc`, vendored `.claude/skills/deck/deck.py` removed (sub-card: goc-migrate-phasor-agents-off-vendored-deckpy)
...
1. **Packaging** (`goc-package-pyproject-and-pypi-release`) — `pyproject.toml`, entry-point wiring, package-data layout for skill templates, PyPI release pipeline.
2. **Install command** (`goc-install-command-scaffolds-repo`) — the `goc install` flow that drops files into a target repo: `.claude/skills/`, `deck/`, hooks, CLAUDE.md sections, validator pre-commit hook.
3. **AGENTS.md output** (`goc-write-agentsmd-alongside-claudemd`) — write/merge AGENTS.md so the methodology is visible to all six major agent runtimes, not just Claude Code.
4. **Multi-agent shims** (`goc-multi-agent-shim-which-agents-at-v1`) — `--agents` flag + per-agent shim templates. **Decision**: which agents at v1 (claude only? +cursor? +codex? +copilot?).
5. **Bootstrap UX** (`goc-bootstrap-error-when-cli-not-on-path`) — when someone clones a GoC-using repo without `goc` installed, skills emit `pipx install game-of-cards` instead of cryptic Python tracebacks.
6. **Dogfood migration** (`goc-migrate-phasor-agents-off-vendored-deckpy`) — phasor-agents itself currently vendors `deck.py`. Once `goc` is on PyPI, this repo deletes the vendored copy and shells to PATH like any other consumer. **Decision**: when (immediately after v1, or wait for v1.1)?
