...
definition_of_done: |
  ...
  - [x] Extend `goc install` / `goc upgrade` so the config is created or migrated without clobbering consuming-repo custom values
  - [x] Teach state-mutating commands (`status`, `advance`, `unadvance`, `decide`, and any future close behavior if applicable) to respect the config while preserving explicit CLI overrides
  - [x] Update AGENTS/CLAUDE skill guidance so auto-commit is described as a configured policy, not a hard-coded invariant
  - [x] Add focused coverage or smoke commands proving both enabled and disabled modes
  - [x] `goc validate` passes after the config changes
---
