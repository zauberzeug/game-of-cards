...
tags: [story, infra]
definition_of_done: |
  - [x] `goc move <old> <new>` no longer recreates `<old>/REDIRECT.md` after the rename; the old card directory is absent after a successful move
  - [x] `load_all_cards` / board rendering no longer need redirect-only directory special cases
  - [x] `goc validate` fails when `deck/` contains a redirect-only card directory or any other stale card-name directory without a valid `README.md`
  - [x] `goc validate` continues to catch stale references in `advances` / `advanced_by` after a move, so redirects are not needed as a compatibility layer
  - [x] Existing `deck/goc-*/REDIRECT.md` directories in this repo are removed and no equivalent redirect directories remain
  - [x] Tests or smoke coverage exercise moving a card, validating a stale relation, and rejecting a redirect-only directory
  - [x] Docs/help text for `goc move` no longer promises a redirect stub
---
