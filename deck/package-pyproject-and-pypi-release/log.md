# Log — package-pyproject-and-pypi-release
...
- Goc repo's deck = goc DEVELOPMENT work post-extraction (new
  ...
  bootstrap.
- Sub-card 6 (`migrate-phasor-agents-off-vendored-deckpy`) is
...
- `goc show package-pyproject-and-pypi-release` → frontmatter
...
2. `goc install` is a stub (returns "not yet implemented"). The
   sub-card `install-command-scaffolds-repo` owns the actual
   scaffold logic.
...
2. `goc install` is a stub. The actual scaffold flow lands when
   sub-card `install-command-scaffolds-repo` ships.
...
Pull-card next tick: probably `install-command-scaffolds-repo` — the
