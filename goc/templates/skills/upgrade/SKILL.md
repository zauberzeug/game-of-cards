...
- Does NOT bisect or `git revert` the engine's writes. If the
  ...
  edits the file by hand or invokes `Skill(create-card)` to file
  the disagreement as a card for follow-up.

## Cross-references

- `Skill(kickoff)` — first-time setup in a fresh repo. The kickoff
...
- `Skill(card-schema)` — frontmatter and DoD conventions the upgraded
  engine may have evolved.
- `Skill(deck)` — the methodology surface the regenerated AGENTS.md /
